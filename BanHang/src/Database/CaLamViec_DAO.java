...
public class CaLamViec_DAO {
	...
	public static int deleteCaLamViec(String macalamviec) throws SQLException, ClassNotFoundException{
		Connection conn = DatabaseManager.getInstance().getConnection();
		int res = 0;
	...
	public static CaLamViec selectCaLamViec(String macalamviec) throws ClassNotFoundException, SQLException {
		Connection conn = DatabaseManager.getInstance().getConnection();
		String sqlQuery = "SELECT * FROM CALAMVIEC WHERE MaCaLamViec = ?";
		PreparedStatement pstmt = conn.prepareStatement(sqlQuery);
		pstmt.setString(1, macalamviec);
